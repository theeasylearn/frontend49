...
const ROUTE = "/travel/"
//get all bus routes 
// localhost:5000/travel
app.get(ROUTE, function (request, response) {
    var query = request.query;
    if (JSON.stringify(query) === "{}") {
        let length = data.length; //3
        response.json([...data, { 'count': length }].toReversed());
    }
    else {
        let from = query.from;
        let to = query.to;
        let type = query.type;
        let lowerprice = query.lowerprice;
        let upperprice = query.upperprice;
        let filterdData = null;
        //get all bus routes between given source & destination & between given price range
        // localhost:5000/travel?from=ahmedbad&to=bhavnagar&lower_price=1000&upper_price=2000
        if (from !== undefined && to !== undefined && lowerprice !== undefined && upperprice != undefined) {
            lowerprice = parseInt(lowerprice);
            upperprice = parseInt(upperprice);
            // response.send(`${from} ${to} ${lower_price}${upper_price}`);
            filterdData = data.filter(function (item) {
                if (item.source.toLowerCase() === from && item.destination.toLowerCase() === to &&
                    item.price >= lowerprice && item.price <= upperprice) {
                    return item;
                };
            });
        }
        ...
        // localhost:5000/travel?from=ahmedbad&to=bhavnagar&price=1000
        */
        else if (from !== undefined && to !== undefined && lowerprice !== undefined) {
            lowerprice = parseInt(lowerprice);
            filterdData = data.filter(function (item) {
                if (item.source.toLowerCase() === from && item.destination.toLowerCase() === to &&
        ...
        /* 
            ...
            // localhost:5000/travel?from=ahmedbad&to=bhavnagar&jouneytype=express
        */
        else if (from !== undefined && to !== undefined && type !== undefined) {
            filterdData = data.filter(function (item) {
                if (item.source.toLowerCase() === from && item.destination.toLowerCase() === to &&
        ...
        /*
            ...
            // localhost:5000/travel?from=ahmedbad&to=bhavnagar
        */
        ...
        if (filterdData != null) {
            let length = filterdData.length; //3
            filterdData.unshift({ 'count': length })
...
app.listen(5000, function (error) {
    if (!error)
        ...
        console.log('server cound not be started');
        console.log(error);
    }
});
